...
RSpec.describe Silas::Configuration do
  describe "boot guard" do
    it "raises a clear removal error when the cut :agent_sdk engine is configured" do
      expect {
        Silas.configure { |c| c.adapter = :agent_sdk }
      }.to raise_error(Silas::BootGuardError, /removed in Silas 0\.2/)
    end

    it "defaults to the :ruby_llm adapter with sensible config" do
      expect(Silas.config.adapter).to eq(:ruby_llm)
      expect(Silas.config.default_model).to be_present
    end
  end

  # The 0.4 rename: "engine" meant both the Rails engine (Silas::Engine) and
  # the inference backend. Old names keep working, loudly, until 2.0 — an
  # upgrade must not break a host app's initializer.
  describe "the engine -> adapter rename (deprecated aliases)" do
    it "config.engine= still sets the adapter, with a warning" do
      expect(Silas.deprecator).to receive(:warn).with(/config\.engine= is deprecated/)
      Silas.config.engine = :ruby_llm
      expect(Silas.config.adapter).to eq(:ruby_llm)
    end

    it "config.engine still reads the adapter, with a warning" do
      Silas.config.adapter = :ruby_llm
      expect(Silas.deprecator).to receive(:warn).with(/config\.engine is deprecated/)
      expect(Silas.config.engine).to eq(:ruby_llm)
    end

    it "Silas.resolved_engine still resolves, with a warning" do
      allow(Silas.deprecator).to receive(:warn)
      Silas.config.adapter = :ruby_llm
      expect(Silas.resolved_engine).to be_a(Silas::Adapters::RubyLLM)
      expect(Silas.deprecator).to have_received(:warn).with(/resolved_engine is deprecated/)
    end

    it "Silas::Engines::Base still resolves for host apps subclassing it" do
      expect(Silas.deprecator).to receive(:warn).with(/Silas::Engines::Base is deprecated/)
      expect(Silas::Engines::Base).to be(Silas::Adapters::Base)
    end
  end
  ...
  describe "re-resolution on reconfigure" do
    ...
    it "re-resolves the engine when it is reconfigured" do
      first = Class.new(Silas::Adapters::Base).new
      second = Class.new(Silas::Adapters::Base).new

      Silas.configure { |c| c.adapter = first }
      expect(Silas.resolved_adapter).to be(first)

      Silas.configure { |c| c.adapter = second }
      expect(Silas.resolved_adapter).to be(second)
    end
